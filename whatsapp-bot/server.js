...
async function getOrCreateSession(sessionId) {
    ...
    const socketConfig = {
        ...
        defaultQueryTimeoutMs: 60000, // 增加查询超时时间
        keepAliveIntervalMs: 30000,
        retryRequestDelayMs: 1000, // 增加重试延迟
        maxMsgRetryCount: 3, // 减少重试次数
        markOnlineOnConnect: true,
        syncFullHistory: false,
        fireInitQueries: true,
        emitOwnEvents: false, // 不触发自己的事件
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            const reason = statusCode || lastDisconnect?.error?.reason || 'unknown';
            const shouldReconnect = statusCode !== DisconnectReason.loggedOut;
            
            console.log(`❌ 机器人 #${sessionId} 连接断开，原因: ${reason}，是否重连: ${shouldReconnect ? '是' : '否'}`);
            
            if (shouldReconnect) {
                // 特殊处理：扫码成功后的重启（错误代码 515 或 428）
                const isRestartRequired = statusCode === 515 || statusCode === 428 || reason === 'restart required';
                
                if (isRestartRequired) {
                    console.log(`🔄 机器人 #${sessionId} 检测到配对成功，正在重启连接...`);
                    await sendStatusToLaravel(sessionId, 'connecting', null, '配对成功，正在连接...');
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            console.error(`❌ 机器人 #${sessionId} 重启失败: ${error.message}`);
                            await sendStatusToLaravel(sessionId, 'error', null, `重启失败: ${error.message}`);
                        }
                    }, 3000); // 3秒后重启
                } else {
                    // 其他断线情况
